...
import { FiUser, FiMail, FiLock, FiPhone, FiMapPin, FiTruck, FiArrowRight, FiArrowLeft } from 'react-icons/fi';
...
export const RegisterVolunteerForm = () => {
  ...
  const onSubmit = async (data) => {
    if (!faceImage) {
      toast.error('Face verification is required for volunteers');
      return;
    }
    ...
    <div style={{ minHeight: '100vh', background: T.bg, position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      >
        <div className="text-center mb-8">
          ...
          >
            <FiTruck size={28} color={T.orange} />
          </motion.div>
          <h1 className="font-display text-3xl font-bold" style={{ color: T.textPrimary }}>Volunteer Hub</h1>
          <p style={{ marginTop: 8, color: T.textSecondary, fontSize: 15 }}>Deliver hope with identity verification</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              <label style={{ fontSize: 13, fontWeight: 600, color: T.textSecondary, marginLeft: 4 }}>Phone</label>
              <div style={{ position: 'relative' }}>
                <FiPhone style={{ position: 'absolute', left: 16, top: 16, color: T.textSecondary }} />
                <input
                  placeholder="+91 9876543210"
                  {...register('phone')}
                  style={{
                    width: '100%', padding: '14px 14px 14px 44px', background: T.bg,
                    border: `1px solid ${errors.phone ? '#ef4444' : T.border}`, borderRadius: 12,
                    color: T.textPrimary, outline: 'none', transition: 'all 0.2s', fontSize: 15
                  }}
                />
              </div>
              {errors.phone && <p style={{ fontSize: 12, color: '#ef4444', marginLeft: 4 }}>{errors.phone.message}</p>}
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label style={{ fontSize: 13, fontWeight: 600, color: T.textSecondary, marginLeft: 4 }}>Base Address</label>
            <div style={{ position: 'relative' }}>
              <FiMapPin style={{ position: 'absolute', left: 16, top: 16, color: T.textSecondary }} />
              <input
                placeholder="123 Local Street, City"
                {...register('address')}
                style={{
                  width: '100%', padding: '14px 14px 14px 44px', background: T.bg,
                  border: `1px solid ${errors.address ? '#ef4444' : T.border}`, borderRadius: 12,
                  color: T.textPrimary, outline: 'none', transition: 'all 0.2s', fontSize: 15
                }}
              />
            </div>
            {errors.address && <p style={{ fontSize: 12, color: '#ef4444', marginLeft: 4 }}>{errors.address.message}</p>}
          </div>
          ...
          <div className="pt-2 border-t mt-6" style={{ borderColor: T.border }}>
            <FaceCapture onCapture={(img) => setFaceImage(img)} existingImage={faceImage} />
          </div>
          ...
          <div className="pt-4">
            ...
            >
              {isSubmitting ? 'Verifying...' : 'Complete Registration'} <FiArrowRight />
            </motion.button>
          </div>
        </form>
